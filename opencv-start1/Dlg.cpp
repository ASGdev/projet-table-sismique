...
void CDlg::ThreadProcess()
{
  ...
  do
  {
    ...
    if(!img_mask_aux.empty())
      cv::imshow("MASK", img_mask_aux);

    if(!img_bgk_aux.empty())
      cv::imshow("BKG", img_bgk_aux);

    if(m_saveFrame.GetCheck() == BST_CHECKED)
    {
      std::string output_filename = "./outputs/input/" + boost::lexical_cast<std::string>(i) + ".png";
      cv::imwrite(output_filename, img_input);
    }
    ...
    if(m_saveMask.GetCheck() == BST_CHECKED)
    {
      ...
      std::string output_filename = "./outputs/foreground/" + boost::lexical_cast<std::string>(i) + ".png";
      cv::imwrite(output_filename, img_mask);
    }
    ...
    if(m_saveBkg.GetCheck() == BST_CHECKED)
    {
      ...
      std::string output_filename = "./outputs/background/" + boost::lexical_cast<std::string>(i) + ".png";
      cv::imwrite(output_filename, img_bkg);
    }
